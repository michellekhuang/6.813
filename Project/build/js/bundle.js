...
/***/ function(module, exports, __webpack_require__) {
	...
	$(document).on('click', '#save-btn', function (event) {
	  ...
	  if (selection.year !== null && selection.month !== null && selection.day !== null && selection.hour !== null && selection.minute !== null) {
	    $('.calendar-container').hide();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var PolyCal = __webpack_require__(4);
	var DateTime = __webpack_require__(5);

	var reservationsDB = __webpack_require__(6);
	...
	var SELECTION = new DateTime(null, null, null, null, null);

	var possibleReservations = ['4:00 pm', '4:30 pm', '5:00 pm', '5:30 pm', '6:00 pm', '6:30 pm', '7:00 pm', '7:30 pm', '8:00 pm', '8:30 pm'];

	var calendar = new PolyCal({ months: 12 });
	var today = calendar.today();

	var thisMonth = calendar.find(today.year, today.month);
	var renderedMonth = calendar.find(today.year, today.month);
	SELECTION.setYear(renderedMonth.year);

	function getDaysArray(month) {
	  var days = month.days;
	  var firstDay = days[0];
	  var lastDay = days[month.days.length - 1];
	  ...
	  for (var _i = 0; _i < days.length; ++_i) {
	    ...
	    daysArray[week].push(days[_i].date);
	  }
	...
	function populateHeader(month) {
	  var monthString = monthsByIndex[month.month];
	  $('#month-label').html(monthString);
	  $('#year-label').html(month.year);
	}

	function populateBody(month) {
	  var daysArray = getDaysArray(month);
	  var html = '';
	  for (var i = 0; i < daysArray.length; ++i) {
	    for (var j = 0; j < daysArray[i].length; ++j) {
	      html += '<div class="cal-box">';
	      if (daysArray[i][j] > -1) {
	        if (month.year === today.year && month.month === today.month && daysArray[i][j] === today.date) {
	        ...
	        } else if (month.month == today.month && daysArray[i][j] > today.date || month.month > today.month) {
	          html += '<div class="day clickable" id="day-' + daysArray[i][j] + '">' + daysArray[i][j] + '</div>';
	        } else {
	          html += '<div class="day nonclickable" id="day-' + daysArray[i][j] + '">' + daysArray[i][j] + '</div>';
	        }
	...
	function timeSlotView(time) {
	  var id = getTimeSlotIdFromTime(time);
	...
	function renderCalendar(month) {
	  ...
	  $('.day.clickable').click(function (event) {
	    ...
	    var clickedDate = parseInt(dayElem.html());
	    ...
	    var openTimes = possibleReservations.slice();

	    var reservations = _.map(reservationsDB.getReservations(), function (reservation) {
	      return parseDate(reservation.date);
	    });
	    for (var i = 0; i < reservations.length; ++i) {
	      if (reservations[i].month === month.month && reservations[i].date === clickedDate) {
	        var index = openTimes.indexOf(reservations[i].time);
	        if (index > -1) {
	          openTimes.splice(index, 1);
	        }
	      }
	    }
	    for (var _i2 = 0; _i2 < openTimes.length; ++_i2) {
	      $('.open-times-container').append(timeSlotView(openTimes[_i2]));
	    }

	    $('.time-slot').click(function (event) {
	      var time = $(event.target).html();
	      SELECTION.setHourMinute(time);
	      $('#save-btn').removeClass('disabled');

	      var timeSlotElems = $('.time-slot');
	      var selectedTimeSlot = $('#' + getTimeSlotIdFromTime(time));
	...
	function getSelection() {
	  return new DateTime(SELECTION.year, SELECTION.month, SELECTION.day, SELECTION.hour, SELECTION.minute);
	}

	function clearSelection() {
	  if (SELECTION.hour) {
	    var timeId = getTimeSlotIdFromTime(SELECTION.getHourMinuteString());
	    var selectedTimeSlot = $('#' + timeId);
	    selectedTimeSlot.removeClass('selected');
	  }
	  if (SELECTION.day) {
	    var selectedDay = $('day-' + SELECTION.date);
	...
	(function (root, factory) {
	    ...
	    function extend(obj) {
	        ...
	        for (i = 0, l = ext.length; i < l; i++) {
	            if (!!ext[i] && (typeof ext === 'undefined' ? 'undefined' : _typeof(ext)) === 'object') {
	                for (key in ext[i]) {
	    ...
	    function Polycal(options) {
	        ...
	        var config = !!options && (typeof options === 'undefined' ? 'undefined' : _typeof(options)) === 'object' ? extend({}, this.defaults, options) : this.defaults;
	        this.model = [];
	        this.init(config);
	    }
	    ...
	    Polycal.prototype.defaults = {
	        ...
	        renderer: function renderer(model) {
	            return model;
	        }
	    ...
	    Polycal.prototype.find = function () {
	        var q = parseDateQuery.apply(null, arguments),
	            i,
	            l,
	            m;
	...
	var DateTime = function () {
	  ...
	  _createClass(DateTime, [{
	    ...
	    value: function isReady() {
	      return this.year !== null && this.month !== null && this.day !== null && this.hour !== null && this.minute !== null;
	    }
	  }, {
	    key: 'setYear',
	    ...
	    key: 'setHourMinute',
	    value: function setHourMinute(time) {
	      if (time === null) {
	        this.hour = null;
	        this.minute = null;
	    ...
	    key: 'getHourMinuteString',
	    value: function getHourMinuteString() {
	      var suffix = 'am';
	      var hour = this.hour;
	    ...
	    value: function toString() {
	      var year = this.year.toString().substring(2, 4);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Reservation = __webpack_require__(9).Reservation;
	var DateTime = __webpack_require__(5);

	var reservations = [new Reservation('Jonatan Yucra', 3, new DateTime(2017, 4, 10, 20, 0), '(456) 789-0123', 'asdf')];
	...
	function addReservation(reservation) {
	  ...
	  reservations.sort(function (res1, res2) {
	    return res1.date.compare(res2.date);
	  });
	  return true;
	}
	...
	function isTimeTaken(date) {
	  var index = _.findIndex(reservations, { date: date });
	  return index !== -1;
	}

	function isNameTaken(name) {
	  var index = _.findIndex(reservations, { name: name });
	  return index !== -1;
	}
...
/***/ function(module, exports) {
	 ...
	 * comments - string
	 */
	var Reservation = function Reservation(name, partySize, date, cell, comments) {
	...
	var Walkin = function Walkin(name, partySize, cell, comments) {
	  _classCallCheck(this, Walkin);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var reservationsDB = __webpack_require__(6);
	var walkinsDB = __webpack_require__(12);
	var seatedDB = __webpack_require__(13);

	var Walkin = form.Walkin;
	...
	$(document).on('click', '#walkin-set-btn', function (evt) {
	  ...
	  var partySize = $('#walkin-party-size').val();
	  var cell = $('#walkin-cell').val();
	  var comments = $('#walkin-comments').val();
	  ...
	  if ($('#walkin-name-error').html() === '') {
	    ...
	    if (walkinsDB.isNameTaken(name) || reservationsDB.isNameTaken(name) || seatedDB.isNameTaken(name)) {
	      ...
	      var result = walkinsDB.addWalkin(walkin);
	      if (result === false) {
	  ...
	  // Populate sidebar
	  var nameElem = $('<div class="data" id="name"></div>');
	  var sizeElem = $('<div class="data" id="size"></div>');
	  var cellElem = $('<div class="data" id="cell"></div>');
	  var commentsElem = $('<div class="data" id="comments"></div>');
	  nameElem.html(walkin.name);
	  sizeElem.html(walkin.partySize);
	  cellElem.html(walkin.cell);
	  commentsElem.html(walkin.comments);

	  var parsedName = walkin.name.replace(' ', '_');
	  var bubble = $('<div class="placeholder" id="' + parsedName + '"></div>');

	  bubble.draggable(utils.draggableOptions);

	  var bubbleHtml = '<button type="button" class="close close-walkin" aria-label="Close"><span aria-hidden="true">&times;</span></button>';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Walkin = __webpack_require__(9).Walkin;

	var walkins = [new Walkin('Eric Chen', 4, '(234) 567-8901', 'pusheen'), new Walkin('Jenny Liu', 6, '(345) 678-9012', 'She has pink hair.')];
	...
	function isNameTaken(name) {
	  var index = _.findIndex(walkins, { name: name });
	  return index !== -1;
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	function getEntry(name) {
	  var dbUsed = void 0;
	  var entry = _.find(walkins, { name: name });
	  dbUsed = "walkins";
	  if (!entry) {
	...
	function isNameTaken(name) {
	  var index1 = _.findIndex(walkins, { name: name });
	  var index2 = _.findIndex(reservations, { name: name });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var sidebarView = __webpack_require__(15);
	...
	$(document).on('click', '#reservation-set-btn', function (event) {
	  ...
	  var partySize = $('#reservation-party-size').val();
	  var date = calendar.getSelection();
	  var cell = $('#reservation-cell').val();
	  ...
	  var reservation = new Reservation(name, partySize, date, cell, comments);
	  if ($('#reservation-name-error').html() === '') {
	    if (walkinsDB.isNameTaken(name) || reservationsDB.isNameTaken(name) || seatedDB.isNameTaken(name)) {
	      ...
	      var result = reservationsDB.addReservation(reservation);
	      if (result === false) {
	        console.log('an unknown error has occurred');
	        inputsValid = false;
	      }
	  ...
	  var nameElem = $('<div class="data" id="name"></div>');
	  var sizeElem = $('<div class="data" id="size"></div>');
	  var dateElem = $('<div class="data" id="date"></div>');
	  var cellElem = $('<div class="data" id="cell"></div>');
	  var commentsElem = $('<div class="data" id="comments"></div>');
	  ...
	  var parsedName = reservation.name.replace(' ', '_');
...
/***/ function(module, exports, __webpack_require__) {
	...
	function getReservationView(reservation) {
	  var parsedName = reservation.name.replace(' ', '_');
	  var bubbleHtml = '<div class="placeholder reservation-placeholder" id="' + parsedName + '">' + '<button type="button" class="close close-reservation" aria-label="Close">' + '<span aria-hidden="true">&times;</span>' + '</button>' + ('<div class="inner-placeholder-name">' + reservation.name + ' (' + reservation.partySize + ')</div>') + ('<span class="inner-placeholder-date">' + reservation.date + '</span>') + '<button type="button" class="btn btn-secondary bubble-btn">Info</button>' + '</div>';
	  return bubbleHtml;
	}

	function renderReservations() {
	  var reservations = reservationsDB.getReservations();
	  var container = $('#reservationContainer');
	  container.html('');
	  for (var i = 0; i < reservations.length; ++i) {
	    var view = getReservationView(reservations[i]);
	    var bubble = $(view);
	    bubble.draggable(utils.draggableOptions);
...
/***/ function(module, exports, __webpack_require__) {
	...
	function placeTable(numSeats, x, y, waiterId) {
	  ...
	  var tableSeats = [2, 4, 6, 8];
	  if ($.inArray(numSeats, tableSeats) != -1) {
	    var tableHtml = '<div class="tables table' + numSeats + ' waiter' + waiterId + '" ' + ('id="' + tableInfo.id + '">') + '<br><p class="app-text">seats</p>' + ('<h3 class="app-text">' + numSeats + '</h3>') + '</div>';
	    var tableId = '#' + tableInfo.id;
	...
	$(document).ready(function () {
	  ...
	  $('.tables').droppable({
	    ...
	    drop: function drop(event, ui) {
	      ...
	      var nameWithUnderscore = draggable[0].id;
	      var originalName = nameWithUnderscore.replace('_', ' ');

	      var name = void 0;
	      var partySize = void 0;
	      var cell = void 0;
	      var comments = void 0;
	      if (walkinsDB.isNameTaken(originalName)) {
	        var walkin = walkinsDB.getWalkin(originalName);
	        ...
	        var reservation = reservationsDB.getReservation(originalName);
	      ...
	      var id = $(this).attr('id');
	      var index = _.findIndex(TABLES, { 'id': id });
	...
	$(document).on('click', '.tables.full', function (event) {
	  ...
	  var seatedEntry = seatedDB.getEntry(name)[0];
	...
	$(document).on('click', '#table-walkin-unseat-btn', function (event) {
	  ...
	  var name = $('#table-walkin-name').html();
	  var size = $('#table-walkin-party-size').html();
	  var cell = $('#table-walkin-cell').html();
	  var comments = $('#table-walkin-comments').html();
	  var entryType = seatedDB.getEntry(name)[1];
	  ...
	  if (entryType == "walkins") {
	    ...
	    var walkin = new Walkin(name, size, cell, comments);
	    walkinsDB.addWalkin(walkin);
	  } else {
	    var reservation = seatedDB.getEntry(name)[0];
	    seatedDB.deleteEntry(name);
	  ...
	  var index = _.findIndex(TABLES, { 'customerName': name });
	  TABLES[index].customerName = null;
	  var id = TABLES[index].id;

	  $('#' + id).html('<br><p class="app-text">seats</p>' + ('<h3 class="app-text">' + TABLES[index].seats + '</h3>'));
...
/***/ function(module, exports, __webpack_require__) {
	...
	$(document).on('click', '.bubble-btn', function (event) {
	  ...
	  if (walkinsDB.isNameTaken(name)) {
	    var walkin = walkinsDB.getWalkin(name);
	    ...
	    var reservation = reservationsDB.getReservation(name);
